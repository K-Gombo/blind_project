...
def insertBoard(title, contents, id, name) :
    board_DAO_logger.info(f"{session['login_info'].get('id')}: blind_board_DAO.insertBoard()에 진입")
    try :    
        con = db_connect()
        cursor = con.cursor()
        ...
        board_DAO_logger.info(f"{session['login_info'].get('id')}: blind_board_DAO.insertBoard()에서 예외 발생({e})")

    board_DAO_logger.info(f"{session['login_info'].get('id')}: blind_board_DAO.inesrtBoard()종료")     
    return result_num
